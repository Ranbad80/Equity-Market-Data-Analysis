...
class Reporter(object):
    ...
    def report(self, spark, trade_date, eod_dir):
        ...
        df = spark.read.parquet(
            "wasbs: // mycontainer @ blobstorageran.blob.core.windows.net/trade/trade_dt={}".format(trade_date))

        # Create Trade Staging Table
        # Use Spark To Read The Trade Table With Date Partition “trade_date”
        df = spark.sql("select symbol, event_tm, event_seq_nb, trade_pr +\
                      from trades  \
                      where trade_dt ='{}'".format(trade_date))
